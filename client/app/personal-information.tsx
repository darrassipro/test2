...
import { useUpdateUserMutation, useGetCurrentUserQuery } from '@/services/userApi';
...
export default function PersonalInformation() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (formData.profileDescription.trim()) updateData.profileDescription = formData.profileDescription.trim();
      if (formData.country.trim()) updateData.country = formData.country.trim();
      
      // Handle images - only include if they're new (not the same as current user images)
      if (profileImage && profileImage !== user?.profileImage) {
        updateData.profileImage = profileImage;
      }
      if (bannerImage && bannerImage !== user?.banner) {
      ...
      updateData.socialMediaLinks = JSON.stringify(linksToSend);
